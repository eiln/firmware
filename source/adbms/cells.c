...
static void print_pack_readings(float pack_volts[MOD_VOLT_NUM])
{
    printf("pack: min: %.3f max: %.3f avg: %.3f delta: %.3f\n",
        pack_volts[MOD_VOLT_MIN], pack_volts[MOD_VOLT_MAX], pack_volts[MOD_VOLT_AVG],
        pack_volts[MOD_VOLT_MAX] - pack_volts[MOD_VOLT_MIN]);
}
...
static void bms_process_cell_readings(void)
{
    ...
    float pack_volts[MOD_VOLT_NUM]; // min, max, avg
    float mod_volts[TOTAL_AD68][MOD_VOLT_NUM];
    ...
    for (int ic = 0; ic < TOTAL_AD68; ic++)
    {
        ...
        mod_volts[ic][MOD_VOLT_MIN] = min;
        mod_volts[ic][MOD_VOLT_MAX] = max;
        mod_volts[ic][MOD_VOLT_AVG] = avg;

        if (!ic)
        {
            pack_volts[MOD_VOLT_MIN] = min;
            pack_volts[MOD_VOLT_MAX] = max;
            pack_volts[MOD_VOLT_AVG] = avg;
        }
        else
        {
            pack_volts[MOD_VOLT_MIN] = MIN(min, pack_volts[MOD_VOLT_MIN]);
            pack_volts[MOD_VOLT_MAX] = MAX(max, pack_volts[MOD_VOLT_MAX]);
            pack_volts[MOD_VOLT_AVG] += avg;
        }
    }
    pack_volts[MOD_VOLT_AVG] /= TOTAL_AD68;

    #define WEAK_PACK_DELTA (10.0f) // V
    for (int ic = 0; ic < TOTAL_AD68; ic++)
    {
        float avg = pack_volts[MOD_VOLT_AVG];
        set = volts < avg && (avg - volts) >= WEAK_PACK_DELTA;
        set |= volts > avg && (volts - avg) >= WEAK_PACK_DELTA;
        bms_set_fault(ic, BMS_ERROR_PACK_WEAK, set);
    }

    print_pack_readings(pack_volts);
}
