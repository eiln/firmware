...
static inline bool is_fault_set(int ic, bms_error_t field)
{
    return (bms.fault[ic] & BMS_GET_ERROR_MASK(field));
}
...
static void print_faults(void)
{
    ...
    for (int ic = 0; ic < TOTAL_AD68; ic++)
    {
        ...
        print_bms_fault(ic, BMS_ERROR_PACK_WEAK);
        ...
        for (int cell = 0; cell < TOTAL_CELL; cell++)
        {
            ...
            print_bms_cell_fault(BMS_ERROR_CELL_WEAK);
        }
...
static bool hard_latch_faults(void)
{
    for (int ic = 0; ic < TOTAL_AD68; ic++)
    {
        if (is_fault_set(ic, BMS_ERROR_VA_UV) &&
            bms_get_fault_duration(ic, BMS_ERROR_VA_UV) >= 5000)
        {
            return true;
        }
    }
...
static void bms_enter_hard_fault(void)
{
    // Pull BMS SDC gpio write
...
{
    if (hard_latch_faults())
    {
        bms_enter_hard_fault();
    }
...
void bms_send_faults(void)
{
    ...
    bms.errors.weak_cell = bms_any_cell_fault(BMS_ERROR_CELL_OV);
