...
// Define cell voltage thresholds
#define MAX_CELL_VOLTAGE 4.20f	   // Absolute max voltage
#define DERATE_START_VOLTAGE 4.15f // Begin limiting above this

// Linear derating curve: from DERATE_START_VOLTAGE to MAX_CELL_VOLTAGE → limit goes from 100% to 0%
static float voltage_ccl_limit(float max_cell_voltage)
{
	if (max_cell_voltage <= DERATE_START_VOLTAGE)
		return 100.0f; // Full regen allowed

	if (max_cell_voltage >= MAX_CELL_VOLTAGE)
		return 0.0f; // No regen allowed

	float slope = -100.0f / (MAX_CELL_VOLTAGE - DERATE_START_VOLTAGE);
	return slope * (max_cell_voltage - DERATE_START_VOLTAGE) + 100.0f;
}

// Compute final regen limit (A)
static float compute_regen_limit(float soc, float temperature, float voltage)
{
	...
	float voltage_limit_a = voltage_ccl_limit(voltage);
	return fminf(fminf(soc_limit_a, temp_limit_a), voltage_limit_a);
}
