...
ClockRateConfig_t clock_config = {
    ...
    .system_clock_target_hz     =TargetCoreClockrateHz,
    .ahb_clock_target_hz        =(TargetCoreClockrateHz / 1),
    .apb1_clock_target_hz       =(TargetCoreClockrateHz / (2)),
    .apb2_clock_target_hz       =(TargetCoreClockrateHz / (2)),
};
#endif
...
#define WAKEUP_DELAY 5       /// 1ms for 2950   /* BMS ic wakeup delay  */

void mdelay2(uint32_t delay)
{
...
void bms_spiTransmitCmd(uint8_t cmd[CMD_LEN])
{
    ...
    PHAL_SPI_transfer_noDMA(&eth_spi_config, txBuff_cmd, CMDPKT_LEN, 0, NULL);
}
...
void bms_spiTransmitData(uint8_t data[TOTAL_IC][DATA_LEN])
{
    ...
    PHAL_SPI_transfer_noDMA(&eth_spi_config,  (uint8_t *)txBuff_data, DATAPKT_LEN * TOTAL_IC, 0, NULL);
}
...
void bms_spiReceiveData(uint8_t rxData[TOTAL_IC][DATA_LEN], uint16_t rxPec[TOTAL_IC], uint8_t rxCc[TOTAL_IC])
{
    ...
    PHAL_SPI_transfer_noDMA(&eth_spi_config, NULL, 0, DATAPKT_LEN * TOTAL_IC, (uint8_t *)rawRxData);
...
int main()
{
    ...
    //PHAL_writeGPIO(BMS_CS2_GPIO_Port, BMS_CS2_Pin, 1);
    //PHAL_writeGPIO(BMS_MSTR_GPIO_Port, BMS_MSTR_Pin, 1);
    //PHAL_writeGPIO(BMS_MSTR2_GPIO_Port, BMS_MSTR2_Pin, 1);
