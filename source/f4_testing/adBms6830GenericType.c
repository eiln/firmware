...
)
{
  ...
  else
  {
    ...
    #if 0
      for (uint8_t current_byte = 0; current_byte < (regData_size); current_byte++)
      {
        rx_data[(current_ic*BYTES_IN_REG)+current_byte] = data[current_byte + (current_ic*BYTES_IN_REG)];
      }
      #endif
      memcpy(&rx_data[current_ic*BYTES_IN_REG], &data[current_ic*BYTES_IN_REG], BYTES_IN_REG);
...
void adBmsReadData(uint8_t tIC, cell_asic *ic, uint8_t cmd_arg[2], TYPE type, GRP group)
{
  ...
  if (tIC > ADBMS_MAX_TIC)
  {
    catch_error();
  }
  uint8_t pec_error[ADBMS_MAX_TIC];
  uint8_t cmd_count[ADBMS_MAX_TIC];
...
)
{
  ...
  uint8_t copyArray[TX_DATA], src_address = 0;
  uint8_t cmd_index;
  #if 0
