...
void sd_write_periodic(void)
{
    ...
    if (dh.sd_state == SD_STATE_FILE_CREATED)
    {
        ...
        //debug_printf("sd: buf count: %d\n", bGetItemCount(&b_rx_can, RX_TAIL_SD));
        if (bGetItemCount(&b_rx_can, RX_TAIL_SD) >= SD_MAX_WRITE_COUNT)
        {
            ...
            {
                //debug_printf("sd tail: con: %d head: %d tail: %d\n", consecutive_items, b_rx_can._head, b_rx_can.tails[RX_TAIL_SD]._tail);
                ...
                {
                    //debug_printf("items read: %d cons: %d\n", bytes_written / sizeof(*buf), consecutive_items);
                    bCommitRead(&b_rx_can, RX_TAIL_SD, bytes_written / sizeof(*buf));
                    //uint32_t end = getTick();
                    //debug_printf("sd write: %d delta1: %d delta2: %d\n", bytes_written / sizeof(*buf), end - start, start - dh.last_write_ms);
                    sd_file_sync(); // fsync takes 4 ticks and makes sure cache is flushed on close
                    //dh.last_write_ms = end;
                }
