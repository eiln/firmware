...
bool uds_init_base(uds_variable_t *uds_tracked_vars_, uint32_t uds_num_vars_)
{
...
static void uds_var_read(uint64_t data)
{
    ...
    uds_frame_send(*(uint64_t *)&msg); // dispatch status
}
...
static void uds_var_read_pin(uint64_t data)
{
    ...
    uds_frame_send(*(uint64_t *)&msg); // dispatch
}
