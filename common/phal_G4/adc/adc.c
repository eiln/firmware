...
bool PHAL_initADC(ADCInitConfig_t *config, ADCChannelConfig_t channels[], uint8_t num_channels)
{
	...
	if (adc == ADC1 || adc == ADC2) {
		...
		RCC->CCIPR |= RCC_CCIPR_ADC12SEL_0; // Select system clock
											// (PCLK) as ADC clock
		...
		RCC->CCIPR &= ~RCC_CCIPR_ADC345SEL;	 // Clear bits
		RCC->CCIPR |= RCC_CCIPR_ADC345SEL_0; // Select system clock
											 // (PCLK) as ADC clock
	...
	adc->CR &= ~ADC_CR_DEEPPWD; // Wake from sleep
	adc->CR |= ADC_CR_ADVREGEN; // Power up internal regulator
