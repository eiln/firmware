...
bool PHAL_initDMA(dma_init_t *dma, uint32_t mem_addr)
{
	...
	// Ensure the stream is disabled, must be in order to configure the DMA
	// control registers
	dma->channel->CCR &= ~(DMA_CCR_EN);
	while (dma->channel->CCR & DMA_CCR_EN)
		;

	// Clear any stream dedicated status flags that may have been set
	// previously
	...
	dma->channel->CCR |= (dma->mem_size << DMA_CCR_MSIZE_Pos) & DMA_CCR_MSIZE_Msk | (dma->periph_size << DMA_CCR_PSIZE_Pos) & DMA_CCR_PSIZE_Msk
						 | (dma->priority << DMA_CCR_PL_Pos) & DMA_CCR_PL_Msk | (dma->mem_inc << DMA_CCR_MINC_Pos) & DMA_CCR_MINC_Msk
						 | (dma->periph_inc << DMA_CCR_PINC_Pos) & DMA_CCR_PINC_Msk | (dma->circular << DMA_CCR_CIRC_Pos) & DMA_CCR_CIRC_Msk
						 | (dma->dir << DMA_CCR_DIR_Pos) & DMA_CCR_DIR_Msk | (dma->tx_isr_en << DMA_CCR_TEIE_Pos) & DMA_CCR_TEIE_Msk
