...
/*
	Useful defines for GPIO Init struct with commonly used peripheral/pin
   mappings. If you find yourself adding the same pin mappings to multiple
   devices, add a macro below to cut down on duplication.
*/

/**
 * @brief Initilize the GPIO perpheral given a list of configuration fields for
 * all of the GPIO pins. Will also enable the GPIO RCC clock
 *
...
inline void PHAL_writeGPIO(GPIO_TypeDef *bank, uint8_t pin, bool value)
{
	bank->BSRR |= 1 << ((!value << 4) | pin); // BSRR has "set" as bottom 16
											  // bits and "reset" as top 16
}
