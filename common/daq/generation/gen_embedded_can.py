...
def gen_filter_lines(lines, rx_msg_configs, peripheral):
    ...
    for msg in rx_msg_configs:
        ...
        if not on_mask:
            ...
            lines.append(f"    {peripheral}->sFilterRegister[{filter_bank}].FR1 = {shift_phrase};\n")
            on_mask = True
        else:
            lines.append(f"    {peripheral}->sFilterRegister[{filter_bank}].FR2 = {shift_phrase};\n")
            on_mask = False
            filter_bank += 1
    lines.append("\n")
...
def configure_node(node_config, node_paths):
    ...
    #
    c_lines = """
// THIS FILE WAS AUTOGENERATED BY common/daq/gen_embedded_can.py

#include "can_parse.h"

can_data_t can_data = {0};
    """
    c_lines = c_lines.splitlines()
    c_lines = [l + "\n" for l in c_lines]

    # Rx switch case
    case_lines = []
    case_lines.append("void handle_rx_autocase(CanMsgTypeDef_t *msg_header) {\n")
    case_lines.append("CanParsedData_t* msg_data_a = (CanParsedData_t *)&msg_header->Data;\n")
    periph = DEFAULT_PERIPHERAL
    if 'can_peripheral' in node_config: periph = node_config['can_peripheral']
    ind = ""
    case_lines.append("\tif (msg_header->Bus == %s){\n" % periph)
    gen_switch_case(case_lines, node_specific_rx_msg_defs, rx_callbacks, ind=ind)
    case_lines.append("        }\n")
    if is_junc:
        periph = junc_config['can_peripheral']
        case_lines.append("\telse if (msg_header->Bus == %s){\n" % periph)
    ...
    # Stale checking
    stale_lines = []
    stale_lines.append("void handle_rx_stale(void) {\n")
    for msg in receiving_msg_defs:
        if msg['msg_period'] > 0:
    ...
    # Hardware filtering
    c_lines.append(ind+"void set_rx_filter(void) {\n")
    filter_lines = []
    if not ("accept_all_messages" in node_config and node_config["accept_all_messages"]):
        ...
        if is_junc:
            periph = junc_config['can_peripheral']
            gen_filter_lines(filter_lines, junc_rx_msg_defs, junc_config['can_peripheral'])
    c_lines += filter_lines
    c_lines.append("}\n")
    ...
    try:
        subprocess.run([
            "clang-format",
            "-style=file",
            f"-assume-filename=/home/eileen/per/firmware/.clang-format",
            "-i",
            node_paths[1]
        ], check=True)
    except subprocess.CalledProcessError:
        print(f"clang-format failed for {node_paths[1]}")
    except FileNotFoundError:
        print("clang-format is not installed or not in PATH.")
