        ...
        for i,daq_node_config in enumerate(daq_bus['nodes']):
            ...
            for can_node in can_bus['nodes']:
                if can_node['node_name'] == daq_node_config['node_name']:
                    ...
                    cmd_msg = {"msg_name":f"uds_command_{name}", "callback":True}
                    can_node['rx'].append(cmd_msg)
                    bl_node['rx'].append(cmd_msg)

                    # configure node tx message
                    rsp_msg = {"msg_name":f"uds_response_{name}",
                                    "msg_desc":f"UDS response from node {daq_node_config['node_name']}",
                                    "signals":[{"sig_name":"payload","type":"uint64_t","length":64}],
                                    "msg_period":0, "msg_hlp":5, "msg_pgn":100 + i}
                    can_node['tx'].append(rsp_msg)
                    bl_node['tx'].append(rsp_msg)

                    # configure daq tx message
                    command_msg = {"msg_name":f"uds_command_{name}",
                           "msg_desc":f"UDS command for node {daq_node_config['node_name']}",
                           "signals":[{"sig_name":"payload","type":"uint64_t","length":64}],
                           "msg_period":0, "msg_hlp":5, "msg_pgn":200 + i}
                    gate_node['tx'].append(command_msg)
                    # configure daq tx message
                    gate_node['rx'].append({"msg_name":f"uds_response_{name}"})
...
""" % (node_config['node_name'].upper(), node_config['node_name'].upper(), warning_msg)
    ...
    if (node_config['node_name'] != "daq"):
        out_lines += "void uds_frame_send(uint64_t data);\n"
    #out_lines += "bool uds_init_base(uds_variable_t *uds_tracked_vars_, uint32_t uds_num_vars_);\n"
